...
export default function ProjectBox({ data , tasks }: ProjectBoxProp) {
  return (
    <div className="min-w-[400px] min-h-[320px]  flex flex-col gap-2 items-start justify-start px-3 py-6 bg-mainColor text-white rounded-lg shadow-md">
      <div className="flex w-full flex-row justify-start items-center gap-5">
        ...
        <div className="flex flex-col">
          <p className="text-2xl font-bold text-white">{data.project.name}</p>
          <p>{data.team.name}</p>
        </div>
      </div>
      <div className="rounded-[15px] bg-[#404348] w-fit p-2 h-[40px] flex flex-row gap-2 items-center justify-start">
          ...
          <div className="flex flex-row gap-2 bg-widgetsColor rounded-[20px] py-1 px-2">
               ...
               <p>{data.team_members.length} </p>
          </div>
          <div className="flex flex-row gap-2 bg-widgetsColor rounded-[20px] py-1 px-2">
               <p>{new Date(data.project.start_time ? new Date(data.project.start_time) : Date.now()).toLocaleString()}</p>
     ...
     <div className="flex flex-row gap-0 justify-start items-start w-full px-0 rounded-lg h-[20px] border">
          {
               tasks.filter((task) => task.task_status.status.toLowerCase() === "done").length > 0 &&
               <div className="flex flex-row justify-center items-center text-white h-full w-fit rounded-lg z-20 px-2"
                    style={{width:`${(tasks.filter((task) => task.task_status.status.toLowerCase() === "done").length/tasks.length)*100}%`,backgroundColor:"#053e00"}}>
                    done
               </div>
          }
          {
               tasks.filter((task) => task.task_status.status.toLowerCase() !== "done").length > 0 &&
               <div className={`flex flex-row justify-center items-center text-white h-full w-fit rounded-r-lg px-4 relative ${tasks.filter((task) => task.task_status.status === "done").length > 0 ? "-left-2 z-10" : ""}`}
                    style={{width:`${(tasks.filter((task) => task.task_status.status !== "done").length/tasks.length)*100}%`,backgroundColor:"#e6a23c"}}>
                    in progress
               </div>
          }
          {
               tasks.filter((task) => task.task_status.status.toLowerCase() === "done").length === 0 &&
               tasks.filter((task) => task.task_status.status.toLowerCase() !== "done").length === 0 &&
               <div className="flex flex-row justify-center items-center text-white h-full w-full rounded-lg px-2"
