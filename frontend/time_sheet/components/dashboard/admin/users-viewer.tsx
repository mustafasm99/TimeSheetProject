      ...
      <ScrollArea className="w-full  overflow-y-auto">
        {adminData.users.map((user) => {
          return (
            <div
              key={user.id}
              className="text-black flex flex-row justify-between items-center w-full px-2 py-2 bg-gray-100 rounded-lg mb-2"
            >
              ...
              <div className="flex flex-col gap-2 justify-between items-center">
                <Button
                  className="w-full bg-red-500 text-white hover:bg-white hover:text-red-500"
                  onClick={async () => {
                    toast.promise(
                      putRequests({
